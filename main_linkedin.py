#!/usr/bin/env python3
"""Entry point for the LinkedIn Automation Agent.
...
Run on a schedule (daemon mode)::
    ...
    python main_linkedin.py run --config linkedin_config.yaml

Execute every job once and exit::

    python main_linkedin.py run --config linkedin_config.yaml --once

Generate a batch of post images / article covers without posting::

    python main_linkedin.py generate --posts 3 --articles 1 --carousels 2
"""
...
def cmd_theme(args: argparse.Namespace) -> None:
    ...
    from linkedin_agent.content_calendar import ThemePlanner
    ...
    print("Run `python main_linkedin.py run` to start the agent and publish on schedule.")


def cmd_calendar(args: argparse.Namespace) -> None:
    """Show the current content calendar."""
    from linkedin_agent.content_calendar import CALENDAR_FILE, ContentCalendar, ThemePlanner
    if not CALENDAR_FILE.exists():
        print("No calendar found. Create one with:\n  python main_linkedin.py theme \"Your theme here\"")
        return
...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    from linkedin_agent.content_generator import ContentGenerator
    generator = ContentGenerator(config.generation)
    results = generator.generate_batch(
        posts=args.posts,
        articles=args.articles,
        carousels=args.carousels,
    )
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    from linkedin_agent.scheduler import AgentScheduler
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_theme.add_argument("--config", default="linkedin_config.yaml")
    ...
    p_gen.add_argument("--articles", type=int, default=0)
    p_gen.add_argument("--carousels", type=int, default=0)
    p_gen.add_argument("--config", default="linkedin_config.yaml")

    # -- flagged -----------------------------------------------------------
    p_flag = sub.add_parser("flagged", help="Show or clear flagged profiles")
    p_flag.add_argument("--clear", action="store_true", help="Clear all flagged profiles")

    # -- run ---------------------------------------------------------------
    p_run = sub.add_parser("run", help="Start the agent scheduler")
    p_run.add_argument("--config", default="linkedin_config.yaml")

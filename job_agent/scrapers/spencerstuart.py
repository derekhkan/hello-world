"""Spencer Stuart executive search scraper."""
...
class SpencerStuartScraper(BaseScraper):
    """Scraper for Spencer Stuart executive search listings."""

    board = JobBoard.SPENCERSTUART
    BASE_URL = "https://www.spencerstuart.com/executive-search/open-positions"
    ...
    def search_jobs(self, preferences: SearchPreferences) -> list[JobListing]:
        """Search Spencer Stuart for executive positions."""
        ...
        try:
            ...
            cards = soup.select(
                ".position, .job-listing, article, .search-result, "
                "[class*='position'], [class*='role'], [class*='job'], "
                ".card, li.result"
            )
            ...
            logger.info(f"Spencer Stuart: found {len(jobs)} matching roles")

        except Exception as e:
            logger.error(f"Spencer Stuart search failed: {e}")
    ...
    def _parse_card(
        ...
        try:
            ...
            if href and not href.startswith("http"):
                href = f"https://www.spencerstuart.com{href}"
            ...
            return JobListing(
                external_id=f"spencerstuart_{external_id}",
                board=JobBoard.SPENCERSTUART,
            ...
            logger.warning(f"Failed to parse Spencer Stuart card: {e}")
            return None
    ...
    def get_job_details(self, job: JobListing) -> JobListing:
        ...
        try:
            ...
            desc_el = soup.select_one(
                ".position-detail, .job-description, article, main, "
                "[class*='content']"
            )
            if desc_el:
                job.description = desc_el.get_text(separator="\n", strip=True)
        except Exception as e:
            logger.warning(f"Failed to get Spencer Stuart details: {e}")
        return job
